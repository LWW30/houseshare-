...
export default function MTDPage() {
  ...
  const loadData = useCallback(async (uid: string, year: number, qi: number) => {
    ...
    setLoading(true)
    setRows([])
    const propsRes = await supabase.from('properties').select('id').eq('user_id', uid)
    const propIds = (propsRes.data || []).map((p: any) => p.id)
    const paymentsRes = propIds.length > 0
      ? await supabase.from('rent_payments').select('amount, due_date').in('property_id', propIds).eq('status', 'paid').gte('due_date', q.start).lte('due_date', q.end)
      : { data: [] }
    const expensesRes = await supabase.from('expenses').select('amount, date, description, category').eq('landlord_id', uid).gte('date', q.start).lte('date', q.end)
    const income: Row[] = (paymentsRes.data || []).map((p: any) => ({ date: p.due_date, description: 'Rental income', category: 'Rental income', amount: Number(p.amount), type: 'income' as const }))
    const expense: Row[] = (expensesRes.data || []).map((e: any) => ({ date: e.date, description: e.description || e.category || 'Expense', category: e.category || 'General', amount: Number(e.amount), type: 'expense' as const }))
  ...
  function exportCSV() {
    ...
    const body = rows.map(r => [r.date, '"' + r.description.replace(/"/g, '""') + '"', '"' + r.category.replace(/"/g, '""') + '"', r.type, r.amount.toFixed(2)].join(',')).join(NL)
    ...
    <Layout>
      <div className="p-6 md:p-8 max-w-4xl mx-auto">
        ...
        <div className="mb-6 p-4 rounded-2xl flex gap-3 border" style={{ background: '#fffbeb', borderColor: '#fcd34d' }}>
          ...
          <p className="text-sm text-amber-800"><span className="font-semibold">MTD for Income Tax is live from April 2026.</span> Landlords with over 50k rental income must submit quarterly digital records to HMRC. Drops to 30k in April 2027.</p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-6">
          {([{ label: 'Income', value: income, color: 'text-green-600', icon: TrendingUp }, { label: 'Expenses', value: expense, color: 'text-red-500', icon: TrendingDown }, { label: 'Net profit', value: profit, color: profit >= 0 ? 'text-blue-600' : 'text-red-500', icon: FileText }] as const).map(({ label, value, color, icon: Icon }) => (
        ...
        <div className="card p-4 mt-6"><p className="text-xs font-semibold uppercase tracking-wider mb-2" style={{ color: 'var(--text-muted)' }}>HMRC submission deadlines</p><p className="text-xs" style={{ color: 'var(--text-muted)' }}>Q1 Apr-Jul by 5 Aug. Q2 Jul-Oct by 5 Nov. Q3 Oct-Jan by 5 Feb. Q4 Jan-Apr by 5 May.</p></div>
      </div>
    </Layout>
  )
}
