...
export default function Dashboard() {
  ...
  if (loading || dataLoading) return (
    <Layout>
      <style>{'.att-flash{animation:attFlash 1.8s ease-in-out infinite}@keyframes attFlash{0%,100%{background:transparent}50%{background:rgba(220,38,38,0.07);border-color:rgba(220,38,38,0.35)}}'}</style>
  ...
  {(() => {
    ...
    return (
      ...
      <div className='grid grid-cols-2 md:grid-cols-5 gap-4 mb-8'>
          ...
          <Link href='/dashboard/payments' className='card p-5 cursor-pointer hover:border-gray-400 transition-colors block' style={overdueCount > 0 ? {animation:'attFlash 1.8s ease-in-out infinite'} : {}}><div className='flex items-center justify-between mb-3'><span className='text-xs font-medium uppercase tracking-wide' style={{ color: 'var(--text-muted)' }}>Collected</span><PoundSterling size={15} className='text-green-500' /></div><div className='text-2xl font-semibold text-green-600'>£{totalCollected.toLocaleString()}</div><div className='text-xs mt-1' style={{ color: 'var(--text-muted)' }}>{collectionRate}% of £{totalExpected.toLocaleString()}</div></Link>
