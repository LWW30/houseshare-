...
export default function Dashboard() {
  ...
  if (loading || dataLoading) return (
    <Layout>
      {overdueCount > 0 && <style>{`@keyframes attention-flash{0%,100%{background:transparent;border-color:transparent}50%{background:rgba(220,38,38,0.08);border-color:rgba(220,38,38,0.4)}}.attention-flash{animation:attention-flash 1.8s ease-in-out infinite}`}</style>}<div className='flex items-center justify-center h-64'><div className='w-6 h-6 border-2 border-gray-200 border-t-gray-900 rounded-full animate-spin' /></div></Layout>
  )
  ...
  {(() => {
    ...
    return (
      ...
      <div className='grid grid-cols-2 md:grid-cols-5 gap-4 mb-8'>
          ...
          <Link href='/dashboard/payments' className={`card p-5 cursor-pointer hover:border-gray-400 transition-colors block${overdueCount > 0 ? ' attention-flash' : ''}`}><div className='flex items-center justify-between mb-3'><span className='text-xs font-medium uppercase tracking-wide' style={{ color: 'var(--text-muted)' }}>Collected</span><PoundSterling size={15} className='text-green-500' /></div><div className='text-2xl font-semibold text-green-600'>£{totalCollected.toLocaleString()}</div><div className='text-xs mt-1' style={{ color: 'var(--text-muted)' }}>{collectionRate}% of £{totalExpected.toLocaleString()}</div></Link>
          ...
          <Link href='/dashboard/payments' className='card p-5 cursor-pointer hover:border-gray-400 transition-colors block'><div className='flex items-center justify-between mb-3'><span className='text-xs font-medium uppercase tracking-wide' style={{ color: 'var(--text-muted)' }}>Attention</span><Bell size={15} className={overdueCount > 0 ? 'text-red-500' : 'text-gray-400'} /></div><div className='text-2xl font-semibold' style={{ color: overdueCount > 0 ? '#dc2626' : 'var(--text-primary)' }}>{attentionCount}</div><div className='text-xs mt-1' style={{ color: 'var(--text-muted)' }}>{overdueCount} overdue</div></Link>
        </div>
