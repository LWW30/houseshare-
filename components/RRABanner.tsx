        ...
        <div className={`mb-6 rounded-2xl px-5 py-4 flex items-center gap-4 border ${isUrgent ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200'}`}>
                ...
                <div className="flex-1 min-w-0">
                        ...
                        <p className={`text-xs leading-relaxed ${isUrgent ? 'text-red-700' : 'text-amber-700'}`}>
                                  ...
                                  <Link href="/dashboard/rra" className="underline font-medium hover:no-underline">Check your compliance checklist</Link>Link>
                        </p>p>
                </div>div>
              <button onClick={dismiss} className={`flex-shrink-0 p-1 rounded-lg transition-colors ${isUrgent ? 'hover:bg-red-100 text-red-400' : 'hover:bg-amber-100 text-amber-400'}`}>
                      <X size={15} />
              </button>button>
        </div>div>
      )
