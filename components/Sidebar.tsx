...
import {
  ...
  BarChart3, FileText, UserCheck, Settings, Banknote,
} from 'lucide-react'
...
const NAV: NavItem[] = [
  ...
  { href: '/dashboard/direct-debit',label: 'Direct Debit',       icon: Banknote, soon: true },
...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Mobile top bar */}
      <div className="md:hidden fixed top-0 left-0 right-0 z-50 bg-gray-900 text-white px-4 flex items-center justify-between"
      ...
      <div className={`md:hidden fixed top-0 left-0 bottom-0 z-50 w-64 bg-gray-900 flex flex-col transition-transform duration-200 ${open ? 'translate-x-0' : '-translate-x-full'}`}
        ...
        <div className="p-3 border-t border-white/10">
          ...
          <div className="px-3 py-1.5 flex items-center justify-between">
            ...
            {!isPro && <Link href="/dashboard/billing" onClick={() => setOpen(false)} className="text-xs text-blue-400">Upgrade ↗</Link>}
          </div>
        </div>
      </div>
      ...
      <aside className="hidden md:flex flex-col w-52 flex-shrink-0 bg-gray-900 min-h-screen">
        <div className="p-3 border-b border-white/10"><Logo /></div>
        <NavLinks />
        <div className="p-3 border-t border-white/10">
          ...
          <div className="px-3 py-1 flex items-center justify-between">
            ...
            {!isPro && <Link href="/dashboard/billing" className="text-xs text-blue-400">Upgrade ↗</Link>}
          </div>
