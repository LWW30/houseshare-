...
const Logo = () => (
    <div className="flex items-center gap-2.5">
        ...
        <span className="font-semibold text-sm tracking-tight">LetFlow</span>span>
    </div>div>
  )
                              ...
                              const active = router.pathname === href || router.pathname.startsWith(href + '/')
                                        return (
                                                    <Link key={href} href={href} onClick={() => setOpen(false)}
                                                                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-colors ${active ? 'bg-white/10 text-white font-medium' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
                                                                >
                                                                <Icon size={16} className="flex-shrink-0" />
                                                                <span className="flex-1 truncate">{label}</span>span>
                                                      {badge && <span className="text-xs px-1.5 py-0.5 rounded-full font-semibold bg-amber-400 text-amber-900 leading-none flex-shrink-0">{badge}</span>span>}
                                                    </Link>Link>
                                                  )
                    })}
                    </>>
                  ...
                  return (
                        <>
                              <aside className="hidden md:flex fixed top-0 left-0 h-full bg-gray-900 text-white flex-col" style={{ width: 'var(--sidebar-width)' }}>
                                      <div className="px-5 py-6 border-b border-gray-800"><Logo /></div>div>
                                      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto"><NavLinks /></nav>nav>
                                      <div className="px-4 py-4 border-t border-gray-800"><div className="text-xs text-gray-500">UK HMO Landlord Portal</div>div></div>div>
                              </aside>aside>
                          ...
                          {open && (
                                  <div className="md:hidden fixed inset-0 z-40 bg-black/60" onClick={() => setOpen(false)}>
                                            <div className="absolute top-0 left-0 h-full w-64 bg-gray-900 pt-16 flex flex-col" onClick={e => e.stopPropagation()}>
                                                        <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto"><NavLinks /></nav>nav>
                                                        <div className="px-4 py-4 border-t border-gray-800"><div className="text-xs text-gray-500">UK HMO Landlord Portal</div>div></div>div>
                                            </div>div>
                                  </div>div>
